import { apiManager } from '../config/gemini';
...
// Retry mechanism with exponential backoff and API switching
const retryWithBackoff = async <T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Try to switch API if it's a quota/rate limit error
      const switched = apiManager.handleAPIFailure(error);
      
      if (switched && attempt < maxRetries) {
        // API switched successfully, retry with new API
        console.log(`🔄 Retrying with new API (attempt ${attempt + 1}/${maxRetries + 1})`);
        continue;
...
export const getRealTimeAlerts = async (language: string): Promise<any[]> => {
  ...
  try {
    ...
    const result = await retryWithBackoff(async () => {
      const currentModel = apiManager.getCurrentModel();
...
export const analyzeCropImage = async (file: File, language: string): Promise<string> => {
  try {
    ...
    const result = await retryWithBackoff(async () => {
      const currentVisionModel = apiManager.getCurrentVisionModel();
      return await currentVisionModel.generateContent([prompt, imagePart]);
    });
